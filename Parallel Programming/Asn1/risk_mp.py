...
def run_simulations(num_simulations, num_armies, attack_dice, defend_dice):
    simulationsToGo = num_simulations
    cpuCount = mp.cpu_count()

    processes = []
    for i in range(cpuCount):
        oneRound = ceil(num_simulations/cpuCount)
        
        if (oneRound < simulationsToGo):
            simsInCore = oneRound
            simulationsToGo -= oneRound
        else:
            simsInCore = simulationsToGo
            simulationsToGo = 0

        
        p = mp.Process(target=simulation, args=(num_armies,attack_dice, defend_dice,
                                                simsInCore, resultsQue))
        processes.append(p)
...
def simulation(num_armies,attack_dice, defend_dice, simsInCore, resultsQue):

        for j in range(simsInCore):
...
def main(num_simulations):
    ...
    run_simulations(num_simulations, num_armies,
